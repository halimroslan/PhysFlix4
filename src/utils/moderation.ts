 ...
 * trolling, cynicism, slang dismissals, and low-effort unconstructive complaints detection.
 * (100% coverage across 12 categories)
 */
const ABUSIVE_PATTERNS: { pattern: RegExp; category: string; reason: string }[] = [
  // 1. Slang Meremehkan / Mengarut ('ntah apa2', 'merapu', 'hauk', 'poyo')
  {
    pattern: /\b(ntah\s*apa2|entah\s*apa2|ntah\s*ape2|entah\s*ape2|ntah\s*apa[- ]*apa|entah\s*apa[- ]*apa|ntah\s*pe[- ]*pe|entah\s*pe[- ]*pe|ntah\s*pape|entah\s*pape|tah\s*ape2|tah\s*apa2|tah\s*pape|apa\s*jadah|apa\s*kejadah|merapu|mengarut|merepek|hauk|cringe|poyo|sembang\s*kari|sembang\s*kencang|acah\s*acah|acah2|krik\s*krik|krik2)\b/i,
    category: "Sinis / Meremehkan",
    reason: "Komen berunsur meremehkan, memperlekehkan ('ntah apa-apa / merapu'), atau slanga tidak sopan tidak dibenarkan.",
  },
  // 2. Hinaan / Insult & Menghina Kualiti Pengajaran
  {
  ...
  // 5. Spam, Sub4Sub & Promosi Channel
  {
  ...
  // 6. Scam / Penipuan Kewangan & Hadiah Palsu
  {
  ...
  // 9. Sinis, Buang Masa & Meremehkan Pengajaran
  {
  ...
  // 10. Keluhan Pasif / 'Tak Faham' Tanpa Soalan Spesifik
  {
  ...
  // 11. Kandungan Seksual / Tidak Sesuai
  {
  ...
  // 12. Komen Tidak Relevan / Spam Ringkas
  {
